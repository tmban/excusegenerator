...
function ExcuseCard({ ex, idx, iCol, c, d, rating, onRate, onSave, alreadySaved, onCopy, copied }) {
  ...
  return (
    <div style={{ background: c.surface, border: `1px solid ${c.border}`, borderRadius: 14, overflow: "hidden" }}>
      <div style={{ padding: "10px 16px", borderBottom: `1px solid ${c.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", background: c.surface2 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ width: 7, height: 7, borderRadius: "50%", background: iCol, flexShrink: 0 }} />
          <span style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: "0.72rem", color: iCol, letterSpacing: "0.05em", textTransform: "uppercase" }}>{ex.tier_label}</span>
          <span style={{ fontSize: "0.68rem", color: c.textFaint }}>Option {idx + 1}</span>
        </div>
        <button onClick={() => setExpanded(!expanded)}
          style={{ background: "none", border: "none", cursor: "pointer", fontSize: "0.7rem", color: c.textFaint, fontWeight: 600, letterSpacing: "0.04em", textTransform: "uppercase" }}>
          {expanded ? "Collapse ↑" : "Expand ↓"}
        </button>
      </div>
      <div style={{ padding: "14px 16px" }}>
        <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: "0.96rem", color: c.text, fontStyle: "italic", lineHeight: 1.45, marginBottom: expanded ? 12 : 0 }}>
          "{ex.short_version}"
        </div>
        {expanded && (
          <>
            <div style={{ fontSize: "0.9rem", color: c.text, lineHeight: 1.75, marginBottom: 12, paddingTop: 12, borderTop: `1px solid ${c.border2}` }}>
              {ex.excuse}
            </div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: 12, marginTop: expanded ? 0 : 12, borderTop: `1px solid ${c.border2}` }}>
          <StarRating value={rating} onChange={onRate} dark={d} />
          <div style={{ display: "flex", gap: 6 }}>
            <button onClick={() => onCopy(expanded ? ex.excuse : ex.short_version, `copy-${ex.id}`)}
  ...
  return (
    <div ref={topRef} style={{ fontFamily: "'DM Sans', sans-serif", background: c.bg, minHeight: "100vh", color: c.text, transition: "background 0.3s, color 0.3s", display: "flex", flexDirection: "column" }}>
      ...
      <header style={{ background: d ? "#0a1c12" : "#007A33", padding: "0 24px" }}>
        <div style={{ maxWidth: 680, margin: "0 auto", padding: "16px 0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div onClick={() => { setExcuses([]); setRatings({}); }} style={{ cursor: "pointer" }}>
            <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: "1.05rem", color: "#fff", letterSpacing: "0.02em" }}>
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "32px 24px 60px", flex: 1, width: "100%" }}>
        ...
        {!loading && excuses.length === 0 && (
          <div className="fade-up">
            <div style={{ marginBottom: 32 }}>
              <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: "2rem", color: c.text, lineHeight: 1.15, marginBottom: 16 }}>
                What do you need<br />an excuse for?
              </div>
              <div style={{ fontSize: "0.88rem", color: c.textMuted }}>The more detail you give, the better your excuses will be. Context is everything.</div>
            </div>
            ...
            <div className="two-col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
              <div>
                ...
                <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Thomas"
              ...
              <div>
                ...
                <input value={recipient} onChange={e => setRecipient(e.target.value)} placeholder="e.g. my boss, my girlfriend"
                  style={{ width: "100%", padding: "12px 14px", borderRadius: 10, border: `1.5px solid ${c.border}`, background: c.inputBg, color: c.text, fontSize: "0.9rem" }} />
              </div>
            </div>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <textarea value={situation} onChange={e => setSituation(e.target.value)}
                placeholder="e.g. I was two hours late to work, I forgot my partner's birthday, I ghosted someone for 3 weeks..."
                rows={4}
            ...
            <div style={{ marginBottom: 32 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 12 }}>
                ...
                <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: "0.85rem", color: iCol }}>
                  {intensity}/10 — {INTENSITY_LABELS[intensity - 1]}
                </div>
              </div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
                <span style={{ fontSize: "0.65rem", color: c.textFaint }}>Very basic</span>
                <span style={{ fontSize: "0.65rem", color: c.textFaint }}>Oscar-worthy</span>
              </div>
            </div>
        ...
        {/* PRIVACY MODAL */}
        {showPrivacy && (
          <div onClick={() => setShowPrivacy(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
            <div onClick={e => e.stopPropagation()} style={{ background: d ? "#0f2619" : "#fff", borderRadius: 16, border: `1px solid ${c.border}`, padding: "26px 26px 22px", maxWidth: 380, width: "100%" }}>
              <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: "1.05rem", color: c.text, marginBottom: 14 }}>🔒 Your privacy</div>
              <div style={{ fontSize: "0.86rem", color: c.textMuted, lineHeight: 1.7, marginBottom: 16 }}>
                Nothing you type here is stored, logged, or saved anywhere by us. Your situation, name, and recipient details are sent directly to generate your excuses and then discarded. We never see them.
              </div>
              <div style={{ fontSize: "0.86rem", color: c.textMuted, lineHeight: 1.7, marginBottom: 20 }}>
                The more specific and detailed you are about your situation, the more accurate and convincing your excuses will be — so don't hold back.
              </div>
              <button onClick={() => setShowPrivacy(false)} style={{ width: "100%", padding: "12px", borderRadius: 10, border: "none", background: c.accent, color: "#fff", fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: "0.88rem", cursor: "pointer" }}>Got it</button>
            </div>
          </div>
        )}
        ...
        {!loading && excuses.length > 0 && (
          <div className="fade-up">
            ...
            <div style={{ marginBottom: 22 }}>
              <div style={{ fontSize: "0.65rem", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: c.textFaint, marginBottom: 5, display: "flex", flexWrap: "wrap", justifyContent: "space-between", gap: 4 }}>
                <span>
                  {excuses[0]?.name ? `${excuses[0].name}'s excuses` : "Your excuses"}
                  {excuses[0]?.recipient ? ` · For ${excuses[0].recipient}` : ""}
                </span>
                <span style={{ color: iCol, fontWeight: 700, whiteSpace: "nowrap" }}>Level {intensity}/10 — {INTENSITY_LABELS[intensity - 1]}</span>
              </div>
            ...
            <div style={{ background: c.surface, border: `1px solid ${c.border}`, borderRadius: 12, padding: "16px", marginBottom: 12 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 10 }}>
                ...
                <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: "0.82rem", color: iCol }}>
                  {intensity}/10 — {INTENSITY_LABELS[intensity - 1]}
                </div>
              </div>
              <div style={{ position: "relative", paddingBottom: 4 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button onClick={() => { setExcuses([]); setRatings({}); setSituation(""); setName(""); setRecipient(""); setIntensity(5); }}
      ...
      <footer style={{ borderTop: `1px solid ${c.border}`, padding: "24px 28px", marginTop: "auto" }}>
        <div style={{ maxWidth: 680, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <div style={{ fontSize: "0.78rem", color: c.textMuted }}>
            Built by{" "}
            <a href="https://thomasmbanefo.com" target="_blank" rel="noopener noreferrer"
              style={{ color: c.textMuted, fontWeight: 600, textDecoration: "none" }}>
              Thomas Mbanefo
            </a>
            {" "}· © 2025
          </div>
          <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
            <button onClick={() => setShowPrivacy(!showPrivacy)}
              style={{ background: "none", border: "none", cursor: "pointer", fontSize: "0.75rem", color: c.textMuted, fontWeight: 500, padding: 0, textDecoration: "none" }}>
              Privacy policy
            </button>
            <a href="https://thomasmbanefo.com" target="_blank" rel="noopener noreferrer"
              style={{ fontSize: "0.75rem", color: c.textMuted, textDecoration: "none", fontWeight: 500 }}>
              Portfolio
            </a>
          </div>
        </div>
